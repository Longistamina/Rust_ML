...
fn main() {
    ...
    let mut guess = String::new(); // use "let mut" to declare a MUTABLE variable to store user input
                                           // String::new() is a function that returns a new instance of a String
                                           // the "::" indicates that "new()" is an association function of String
                                           // so, this String::new() returns an empty string which the variable "guess" is bound to

    io::stdin() // or can use std::io::stdin() if we had not imported std::io before
        .read_line(&mut guess) 
        .expect("Failed to read line");

    // .read_line(&mut guess) call out read_line() from stdin() to take the user input, stored into "guess"
    // that's why "guess" must be mutable so that the method .read_line() can change its content to take in user input
    ...
    // reference is also immutable by default, that's why we write "&mut guess", not "&guess"
    ...
    // one-liner: io::stdin().read_line(&mut guess).expect("Failed to read line");

    println!("You guessed: {guess}"); // The {} is a placeholder, to print out the value of the "guess" variable
}

/*
╰─ $ cargo run -p part_01_rust_programming --bin 02_io_GuessingString
    Finished `dev` profile [unoptimized + debuginfo] target(s) in 0.06s
     Running `target/debug/02_io_GuessingString`
Guess the number!
Please input your guess.
Rust is so cool!!!!
You guessed: Rust is so cool!!!!
*/